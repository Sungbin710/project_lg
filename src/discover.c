...
static void on_finished_cb(GstDiscoverer * discoverer, Discover_Data * data)
{
...
int print_media_info()
{
	...
	char file_path[256];
	long file_size;
	if (print_media_info_choice(file_path, sizeof(file_path)) == 1) {
		return 1;
	}
	strcat(uri, file_path);

	// printf("disc_file_path: %s\n", file_path);
	// printf("disc_uri: %s\n", uri);
...
int is_playable(const gchar * uri)
{
	...
	info = gst_discoverer_discover_uri(data.discoverer, uri, &err);;
	result = gst_discoverer_info_get_result(info);

	g_object_unref(data.discoverer);

	if (result == GST_DISCOVERER_OK) {
		g_print("%s is playable\n", uri);
		return 0;
	}

	g_print("%s is not playable\n", uri);
	return 1;
}
