...
기능 별 구현내용이 단순하기 때문에 `FR01`, `FR02`와 같은 각각의 기능을 하나의 모듈로 가정하고 모듈테스트를 진행한다. 
각각의 모듈이 테스트를 마치고나면, 전체 시스템에 점진적으로 모듈을 추가해가며 통합테스트를 진행한다.
...
`FR01`, `FR02`, `FR04`은 모두 서버와 동일한 통신과정(송/수신)을 거친다. 즉, 하나의 더미 서버로 각각의 기능에 대해 유닛 테스트를 진행할 수 있다.


1. `FR01-1`: 회원가입을 수행한다.
<img src="./images/test_plan1.png" width="600" height="350"/>


-  테스트 시나리오
   1. 사용자 정보 형식 검증
      -  사용자가 입력한 정보의 형식이 입력받고자 하는 형식과 일치하는지
      -  일치하지 않는다면 회원 가입을 요청하기 전 상태로 이동하는지
      -  일치한다면 다음 단계로 넘어가는지
   2. 사용자 등록 요청
      -   사용자 정보가 서버로 잘 전송되었는지 (Output)
   3. 서버로부터 사용자 등록 승인 여부 수신
      -  사용자 등록 승인 여부가 잘 수신되었는지
      -  수신이 1초 이상 걸린다면 타임아웃이 출력되는지
   4. 사용자 등록 승인 여부 출력
      -  수신된 정보에 따라 사용자에게 등록 승인 여부가 출력되는지 (Output)
...
2. `FR02-1`: 사용자가 로그인 한다.
<img src="./images/test_plan2.png" width="600" height="350"/>
...
-  테스트 시나리오
   1. 사용자 ID 형식 검증
      ...
      -  일치한다면 다음 단계로 넘어가는지
   2. 사용자 ID 확인 요청
      -   사용자 ID가 서버로 정확히 전송되었는지
   3. 사용자 등록 승인 여부 수신
      -  사용자 ID의 등록 여부가 정확히 수신되었는지
      -  수신이 1초 이상 걸린다면 타임아웃이 출력되는지
   4. 사용자 로그인 성공 여부 출력
...
3. `FR04-1`: 사용자 기반 정보가 정상적으로 출력되는지 확인한다.
<img src="./images/test_plan4.png" width="600" height="350"/>
-  테스트 시나리오
   -  사용자 정보 요청
      -  사용자 ID가 서버로 잘 전송되었는지
   -  사용자 정보 수신
      -  사용자 정보가 정확히 수신되었는지
   -  사용자 정보 출력
      -  사용자 정보가 정확히 출력되는지

   -  success: cmd값이 [0x04]로 들어오면 서버로부터 사용자 정보를 정상적으로 수신했다고 생각하고, 사용자 정보를 읽고. 출력되는지 확인한다.
   -  failure: cmd값이 [0x04]와 다르다면 잘못된 수신을 했다고 생각하고, 오류가 발생했음을 출력한다.

1. `FR05-1`: 사용자가 로그아웃되어 state 1로 이동하는지 확인한다.
   -  state 1로 이동되어 사용자 등록과 로그인 기능을 사용할 수 있음이 표시되는 것을 확인한다.
